...
export default class ProfileContainer extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      activeIndex: 0,
      token: '',
      id: this.props.navigation.getParam('profile_id'),
      my_id: '',
      active: false,
      nbr_followers: '',
      im_following: bool,
  ...
  static navigationOptions = ({navigation, screenProps}) => ({
    ...
    headerRight: (
      <TouchableOpacity style={{paddingHorizontal: 12}}>
        <Ionicon name="ios-menu" size={24} color="#52575D"></Ionicon>
      </TouchableOpacity>
    ),
  });
  ...
  async getInfo() {
    var url = website_url + 'api/account/' + this.state.id + '/';

    let token = await AsyncStorage.getItem('res');
    let my_id = await AsyncStorage.getItem('my_id');
    ...
    try {
      ...
      if (response.status >= 200 && response.status <= 300) {
        this.setState({
          ...
          nbr_followers: res.profile.get_number_of_followers,
          loading: false,
        });
      } else {
        let error = res;
  ...
  async follow() {
    var url = website_url + 'api/follow/' + this.state.id + '/';
    let token = await AsyncStorage.getItem('res');
    const tokens = JSON.parse(token);
    let data = {
      method: 'POST',
    ...
    let res = await response.json();
    if (response.status >= 200 && response.status <= 300) {
      // let newData = res.im_following;
      // newData = true;
      this.state.nbr_followers = this.state.nbr_followers + 1;
      this.setState({im_following: true});
    } else {
      let error = res;
      throw error;
    }
  ...
  async unfollow() {
    var url = website_url + 'api/unfollow/' + this.state.id + '/';
    let token = await AsyncStorage.getItem('res');
    const tokens = JSON.parse(token);
    let data = {
      method: 'POST',
    ...
    if (this.state.im_following) {
      let response = await fetch(url, data);
      let res = await response.json();
      if (response.status >= 200 && response.status <= 300) {
        let newData = this.state.im_following;
        newData = false;
        this.state.nbr_followers = this.state.nbr_followers - 1;
        this.setState({im_following: newData});
      } else {
        let error = res;
        throw error;
      }
  ...
  render() {
    ...
    const request_user = this.state.my_id;
    return (
      <SafeAreaView style={styles.container}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={{alignSelf: 'center', marginTop: -83}}>
            <BoxShadow setting={shadowOpt}>
              <View style={styles.profileImage}>
                {this.state.profile.mugshot ? (
                  <Image
                    source={{
                      uri: this.state.profile.mugshot,
                    }}
                    style={styles.image}
                    resizeMode="cover"
                  />
          ...
          <View style={styles.infoContainer}>
            ...
            {request_user != this.state.infos.id ? (
              <TouchableOpacity
                onPress={() => {
                  this.state.followers.includes(2)
                    ? this.follow(this.state.info)
                    : this.unfollow(this.state.info);
                }}
                style={[
                  styles.followbutton,
                  {
                    backgroundColor: this.state.followers.includes(2)
                      ? '#fff'
                      : '#00ace6',
                  },
                ]}>
            ...
            <Text
              ...
              {this.state.overview != null ? this.state.overview.content : null}
            </Text>
          ...
          <View style={styles.statsContainer}>
            <View style={styles.statsBox}>
              <Text
                style={[styles.textSegond, {fontSize: 18, fontWeight: '900'}]}>
                48
              </Text>
