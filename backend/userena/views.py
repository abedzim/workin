...
from userena.forms import (SignupFormTos, SignupFormOnlyEmail, AuthenticationForm,
                           ChangeEmailForm, EditProfileForm, EditPhotoForm)
...
@secure_required
def signup(request, signup_form=SignupFormTos,
           template_name='auth/signup.html', success_url=None,
           extra_context=None):
    """
...
def signin(request, auth_form=AuthenticationForm,
           template_name='auth/landing.html',
...
def profile_edit(request, username, edit_profile_form=EditProfileForm,
                 template_name='userena/edit-profile.html', success_url=None,
...
def photo_edit(request, username, edit_profile_form=EditPhotoForm,
                 template_name='change_photo.html', success_url=None,
                 extra_context=None, **kwargs):
        ...
        if form.is_valid():
            ...
            if success_url:
                ...
                userena_signals.profile_change.send(sender=None,
                                                    user=user)
                redirect_to = success_url
            else: redirect_to = reverse('show_profile', kwargs={'username': username})
            return redirect(redirect_to)
...
def profile_detail(request, username,
    ...
    **Context**
    ...
    """
    user = get_object_or_404(get_user_model(), username__iexact=username)
    nws = Share.objects.filter(reply=False, user=user)
    profile = get_user_profile(user=user)
    if not profile.can_view_profile(request.user):
        raise PermissionDenied
    if not extra_context: extra_context = dict()
    extra_context['nws'] = nws
    extra_context['profile'] = profile
