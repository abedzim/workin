...
class QaDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(QaDetailView, self).get_context_data(**kwargs)
        context['form'] = AnswerForm
        return context

def voirqa(request, id=id):
    qa = get_object_or_404(Question, id=id)
    return render(request, 'qa-detail.html', {"qa": qa})
...
class CreateAnswerView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        messages.success(self.request, self.message)
        return reverse(
            "qa:detail", kwargs={"pk": self.kwargs["question_id"]})
